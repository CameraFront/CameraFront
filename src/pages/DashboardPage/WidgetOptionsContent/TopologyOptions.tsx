...
const TopologyOptions = ({ id, data }: Props) => {
  const { message } = App.useApp();
  const [form] = Form.useForm<TopologyWidgetData['options']>();
  const [
    updateWidgetOptions,
  ...
  const onValuesChange = useCallback(
    (
      changedValues: Partial<TopologyWidgetData['options']>,
      allValues: TopologyWidgetData['options'],
    ) => {
  ...
  const mapOptions = useMemo(() => {
    ...
    return topologyMapList?.map(map => {
      ...
      const label = `${map.topologyNm}(${parentNames?.join('>')})`;
  ...
  return (
    <Form<TopologyWidgetData['options']>
      form={form}
      layout="horizontal"
      name="topologyOptionsForm"
      autoComplete="off"
