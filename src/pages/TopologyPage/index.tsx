...
import { resetState } from '@/features/topologyPage/topologySlice';
import { useGetTopologyMapListQuery } from '@/services/api/topology';
import SearchableTree from './SearchableTree';
import ArtBoard from './topologyEditor/ArtBoard';
import Header from './topologyEditor/Header';

const TopologyPage = () => {
  const dispatch = useAppDispatch();
  const { isFullScreenMode } = useAppSelector(store => store.topology);
  const { isLoading } = useGetTopologyMapListQuery();
  const { isAlarmOn, isAudibleOn, isDarkMode, isAlarmPopupOn } = useAppSelector(
    store => store.global,
  );
